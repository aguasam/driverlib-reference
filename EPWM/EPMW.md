...
* **Parâmetros**:
    ...
    * `emulationMode`: Especifica o modo de emulação. Deve ser um dos valores do enum `EPWM_EmulationMode`:
        * `EPWM_EMULATION_FREE_RUN`: O contador EPWM continua a funcionar no modo de emulação.
        * `EPWM_EMULATION_STOP_AFTER_NEXT`: O contador EPWM para após o próximo ciclo no modo de emulação.
        * `EPWM_EMULATION_STOP_IMMEDIATELY`: O contador EPWM para imediatamente no modo de emulação.
...
* **Exemplo**:
    ...
    ```
    Este exemplo configura o EPWM1 para parar seu contador após o ciclo atual quando o depurador pausa a CPU.
...
* **A Estrutura `EPWM_SignalParams`**:
    ...
    typedef struct
    {
        float32_t sysClkInHz; // Frequência do clock do sistema em Hz
        float32_t freqInHz; // Frequência PWM desejada em Hz
        EPWM_TimeBaseClockDivider tbClkDiv; // Divisor do clock da base de tempo
        EPWM_TimeBaseHighSpeedClockDivider tbHSClkDiv; // Divisor do clock de alta velocidade da base de tempo
        EPWM_CounterMode tbCtrMode; // Modo de contador da base de tempo
        float32_t dutyValA; // Valor do ciclo de trabalho para EPWMxA (0.0 a 1.0)
        float32_t dutyValB; // Valor do ciclo de trabalho para EPWMxB (0.0 a 1.0)
        bool invertSignalB; // Invertir a saída EPWMxB (true ou false)
    } EPWM_SignalParams;
    ```
...
* **Exemplo**:
    ...
    myEPWMConfig.sysClkInHz = 200e6; // Clock do sistema de 200 MHz
    myEPWMConfig.freqInHz = 10e3; // Frequência PWM de 10 kHz
    ...
    myEPWMConfig.dutyValA = 0.25f; // Ciclo de trabalho de 25% para EPWMxA
    myEPWMConfig.dutyValB = 0.75f; // Ciclo de trabalho de 75% para EPWMxB
    myEPWMConfig.invertSignalB = false;

    EPWM_configureSignal(EPWM2_BASE, &myEPWMConfig);
    ```
    Este exemplo configura o EPWM2 para gerar um sinal PWM de 10 kHz com ciclo de trabalho de 25% em `EPWMxA` e 75% em `EPWMxB`, usando o modo de contagem crescente-decrescente e um clock de sistema de 200 MHz.
...
* **Propósito**: Configura os divisores de clock para o contador da base de tempo do módulo EPWM. Isso permite controlar a frequência do clock de contagem do temporizador, que por sua vez afeta o período máximo e a resolução do PWM.
* **Sintaxe**:
    ```c
...
* **Parâmetros**:
    ...
    * `tbClkDiv`: O divisor do clock da base de tempo principal. Deve ser um dos valores do enum `EPWM_TimeBaseClockDivider` (e.g., `EPWM_CLOCK_DIVIDER_1`, `EPWM_CLOCK_DIVIDER_2`, etc.).
    * `tbHSClkDiv`: O divisor do clock de alta velocidade da base de tempo. Deve ser um dos valores do enum `EPWM_TimeBaseHighSpeedClockDivider` (e.g., `EPWM_HSCLOCK_DIVIDER_1`, `EPWM_HSCLOCK_DIVIDER_2`, etc.).
* **Valor de Retorno**: Nenhum (função void).
* **Descrição**: Esta função configura os bits `CLKDIV` e `HSPCLKDIV` no registrador `TBCTL` do EPWM. A frequência final do clock da base de tempo é determinada pela divisão do clock do sistema por esses dois prescalers. Uma frequência de clock da base de tempo mais baixa permite períodos de PWM mais longos, enquanto uma frequência mais alta oferece maior resolução.
* **Exemplo**:
    ```c
...
* **Parâmetros**:
    * `base`: O endereço base do módulo EPWM (e.g., `EPWM1_BASE`).
    * `counterMode`: O modo de contagem desejado. Deve ser um dos valores do enum `EPWM_CounterMode`:
...
* **Parâmetros**:
    * `base`: O endereço base do módulo EPWM (e.g., `EPWM1_BASE`).
    * `period`: O valor do período para o contador da base de tempo. Este valor é carregado no registrador `TBPRD`. O valor máximo é 0xFFFF (65535).
* **Valor de Retorno**: Nenhum (função void).
* **Descrição**: Esta função escreve o valor fornecido no registrador `TBPRD` (Time Base Period) do EPWM. O contador da base de tempo (`TBCTR`) conta de 0 até este valor (no modo crescente), ou de `period` até 0 (no modo decrescente), ou de 0 até `period` e de volta para 0 (no modo crescente-decrescente). A frequência do PWM é inversamente proporcional ao valor do período e à frequência do clock da base de tempo.
* **Exemplo**:
    ```c
...
* **Parâmetros**:
    * `base`: O endereço base do módulo EPWM (e.g., `EPWM1_BASE`).
    * `phaseShift`: O valor de deslocamento de fase. Este valor é carregado no registrador `TBPHS`. O valor máximo é 0xFFFF (65535).
* **Valor de Retorno**: Nenhum (função void).
...
* **Parâmetros**:
    * `base`: O endereço base do módulo EPWM (e.g., `EPWM1_BASE`).
    * `counterValue`: O valor para o qual o contador da base de tempo (`TBCTR`) será definido. O valor máximo é 0xFFFF (65535).
* **Valor de Retorno**: Nenhum (função void).
* **Descrição**: Esta função escreve o `counterValue` fornecido diretamente no registrador `TBCTR` (Time Base Counter) do EPWM. Definir o contador da base de tempo pode ser útil para:
...
* **Propósito**: Configura o modo de carregamento sombra para os registradores de comparação (`CMPA` e `CMPB`) do módulo EPWM. O carregamento sombra garante que as atualizações dos valores de comparação ocorram de forma síncrona com os eventos do temporizador, evitando glitches nas formas de onda PWM.
* **Sintaxe**:
    ```c
...
* **Parâmetros**:
    ...
    * `compModule`: O módulo de comparação a ser configurado. Deve ser um dos valores do enum `EPWM_CounterCompareModule`:
        * `EPWM_COUNTER_COMPARE_A`: Para o registrador de comparação A (`CMPA`).
        * `EPWM_COUNTER_COMPARE_B`: Para o registrador de comparação B (`CMPB`).
    * `loadMode`: O modo de carregamento sombra. Deve ser um dos valores do enum `EPWM_LoadMode`:
        * `EPWM_COMP_LOAD_ON_CNTR_ZERO`: Carrega o valor sombra quando o contador da base de tempo atinge zero.
        * `EPWM_COMP_LOAD_ON_CNTR_PRD`: Carrega o valor sombra quando o contador da base de tempo atinge o período.
        * `EPWM_COMP_LOAD_ON_CNTR_ZERO_OR_PRD`: Carrega o valor sombra quando o contador atinge zero ou o período.
        * `EPWM_COMP_LOAD_FREEZE`: O carregamento do valor sombra é congelado.
* **Valor de Retorno**: Nenhum (função void).
* **Descrição**: Esta função configura os bits `CMPC` e `CMPB` no registrador `CMPCTL` do EPWM. Ao utilizar o carregamento sombra, você garante que as alterações nos valores de comparação não afetem a forma de onda PWM imediatamente, mas sim no próximo evento de carregamento especificado (ZERO, PRD ou ambos). Isso é crucial para manter a integridade da forma de onda e evitar transições indesejadas.
* **Exemplo**:
    ```c
    ...
    EPWM_setCounterCompareShadowLoadMode(EPWM2_BASE,
                                         ...
                                         EPWM_COMP_LOAD_ON_CNTR_PRD);
    ```

### 2.8 `EPWM_setCounterCompareValue()`

* **Propósito**: Define o valor de comparação para um dos registradores de comparação (`CMPA` ou `CMPB`) do módulo EPWM. Esses valores são cruciais para determinar o ciclo de trabalho e as transições de borda dos sinais PWM gerados.
* **Sintaxe**:
    ```c
...
* **Parâmetros**:
    ...
    * `compModule`: O módulo de comparação a ser configurado. Deve ser um dos valores do enum `EPWM_CounterCompareModule`:
        * `EPWM_COUNTER_COMPARE_A`: Para o registrador de comparação A (`CMPA`).
        * `EPWM_COUNTER_COMPARE_B`: Para o registrador de comparação B (`CMPB`).
    * `compareValue`: O valor a ser carregado no registrador de comparação. Este valor é comparado com o contador da base de tempo (`TBCTR`) para gerar eventos de acionamento. O valor máximo é 0xFFFF (65535).
* **Valor de Retorno**: Nenhum (função void).
* **Descrição**: Esta função escreve o `compareValue` fornecido no registrador `CMPA` ou `CMPB` do EPWM, dependendo do `compModule` selecionado. Os valores de comparação são usados pelos qualificadores de ação para determinar quando a saída PWM deve mudar de estado (por exemplo, de alta para baixa ou vice-versa). A relação entre o valor de comparação e o período da base de tempo (`TBPRD`) define o ciclo de trabalho do sinal PWM. É importante notar que, se o carregamento sombra estiver habilitado para o módulo de comparação, o `compareValue` será escrito no registrador sombra e só será transferido para o registrador ativo no próximo evento de carregamento sombra.
* **Exemplo**:
    ```c
...
* **Parâmetros**:
    * `base`: O endereço base do módulo EPWM (e.g., `EPWM1_BASE`).
    * `output`: A saída EPWM a ser configurada. Deve ser um dos valores do enum `EPWM_ActionQualifierOutput`:
        * `EPWM_AQ_OUTPUT_A`: Para a saída `EPWMxA`.
        * `EPWM_AQ_OUTPUT_B`: Para a saída `EPWMxB`.
    * `action`: A ação a ser tomada na saída quando o evento ocorrer. Deve ser um dos valores do enum `EPWM_ActionQualifierOutputAction`:
    ...
    * `event`: O evento que aciona a ação. Deve ser um dos valores do enum `EPWM_ActionQualifierEvent`:
